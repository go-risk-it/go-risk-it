...
func TestError_StaleState_AdvanceFails3x_Fatal(t *testing.T) {
	...
	ctx := context.Background()
	rest := &fakeRESTForError{advanceErr: fmt.Errorf("advance failed")}
	h, _ := makeErrorHandler(ctx, rest)
	bus := NewTestBus()
	h.Register(bus)
	...
	assert.NotNil(t, completes[0].(GameCompleteEvent).Result.FatalError)
}
...
func TestError_Execution_PlayCards_TriesAdvance(t *testing.T) {
	...
	bus.Emit(MoveFailedEvent{
		...
		Err:     fmt.Errorf("card play failed"),
		ErrType: "execution",
	})
