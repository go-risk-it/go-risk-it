...
func (s *Server) serveConn(c net.Conn, opts *ServeConnOpts, newf func(*serverConn)) {
	...
	sc := &serverConn{
		...
		wroteFrameCh: make(
			chan frameWriteResult,
			1,
		), // buffered; one send in writeFrameAsync
		bodyReadCh: make(
			chan bodyReadMsg,
		), // buffering doesn't matter either way
		...
		*sc.tlsState = tc.ConnectionState()
		...
		if !conf.PermitProhibitedCipherSuites && isBadCipher(sc.tlsState.CipherSuite) {
			...
			sc.rejectConn(
				...
				fmt.Sprintf("Prohibited TLS 1.2 Cipher Suite: %x", sc.tlsState.CipherSuite),
			)
			return
		}
	}
...
func (sc *serverConn) serve(conf http2Config) {
	...
	if err := sc.readPreface(); err != nil {
		sc.condlogf(
			err,
			"http2: server: error reading preface from client %v: %v",
			sc.conn.RemoteAddr(),
			err,
		)
		return
	...
	for {
		...
		if sentGoAway && sc.shutdownTimer == nil &&
			(sc.goAwayCode != ErrCodeNo || gracefulShutdownComplete) {
...
func (sc *serverConn) processData(f *DataFrame) error {
	...
	if st.declBodyBytes != -1 && st.bodyBytes+int64(len(data)) > st.declBodyBytes {
		...
		st.body.CloseWithError(
			fmt.Errorf(
				"sender tried to send more than declared Content-Length of %d bytes",
				st.declBodyBytes,
			),
		)
...
func (st *stream) endStream() {
	...
	if st.declBodyBytes != -1 && st.declBodyBytes != st.bodyBytes {
		st.body.CloseWithError(
			fmt.Errorf("request declared a Content-Length of %d but only wrote %d bytes",
...
func (sc *serverConn) upgradeRequest(req *http.Request) {
	...
	st := sc.newStream(
		...
		defaultRFC9218Priority(sc.priorityAware && !sc.hasIntermediary),
	)
	st.reqTrailer = req.Trailer
	if st.reqTrailer != nil {
		st.trailer = make(http.Header)
...
func (sc *serverConn) processPriorityUpdate(f *PriorityUpdateFrame) error {
	...
	if !ok {
		return sc.countError(
			"unparsable_priority_update",
			streamError(f.PrioritizedStreamID, ErrCodeProtocol),
		)
	}
...
func (sc *serverConn) newWriterAndRequest(
	st *stream,
	f *MetaHeadersFrame,
...
func (rws *responseWriterState) writeChunk(p []byte) (n int, err error) {
	...
	if !rws.sentHeader {
		...
		if !hasContentLength && clen == "" && rws.handlerDone &&
			bodyAllowedForStatus(rws.status) &&
			(len(p) > 0 || !isHeadResp) {
...
var (
	...
	ErrPushLimitReached = errors.New(
		"http2: push would exceed peer's SETTINGS_MAX_CONCURRENT_STREAMS",
	)
)
