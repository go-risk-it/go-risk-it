...
var (
	...
	errClientConnGotGoAway      = errors.New(
		"http2: Transport received Server's graceful shutdown GOAWAY",
	)
	errClientConnForceClosed = errors.New(
...
func (t *Transport) dialClientConn(
	ctx context.Context,
	addr string,
	singleUse bool,
) (*ClientConn, error) {
...
func (t *Transport) newClientConn(
	...
	initialSettings = append(
		initialSettings,
		Setting{ID: SettingMaxFrameSize, Val: conf.MaxReadFrameSize},
	)
	...
	if maxHeaderTableSize != initialHeaderTableSize {
		initialSettings = append(
			initialSettings,
			Setting{ID: SettingHeaderTableSize, Val: maxHeaderTableSize},
		)
	}
	...
	for streamID, cs := range cc.streams {
		...
		if streamID == 1 && cc.goAway.ErrCode != ErrCodeNo {
			...
			cs.abortStreamLocked(
				fmt.Errorf(
					"http2: Transport received GOAWAY from server ErrCode:%v",
					cc.goAway.ErrCode,
				),
			)
		} else {
...
func (cc *ClientConn) tooIdleLocked() bool {
	...
	return cc.idleTimeout != 0 && !cc.lastIdle.IsZero() &&
		time.Since(cc.lastIdle.Round(0)) > cc.idleTimeout
}
...
func (cc *ClientConn) writeHeaders(
	streamID uint32,
	endStream bool,
	maxFrameSize int,
	hdrs []byte,
...
func (rl *clientConnReadLoop) handleResponse(
	cs *clientStream,
	f *MetaHeadersFrame,
) (*http.Response, error) {
	if f.Truncated {
...
var (
	errResponseHeaderListSize = errors.New(
...
func (rt erringRoundTripper) RoundTrip(
	*http.Request,
) (*http.Response, error) {
	return nil, rt.err
}
...
func (rt noDialH2RoundTripper) NewClientConn(
	conn net.Conn,
	internalStateHook func(),
...
// connection.
func (t *Transport) dialTLSWithContext(
	ctx context.Context,
	network, addr string,
	cfg *tls.Config,
) (*tls.Conn, error) {
