...
func (n *priorityNodeRFC7540) walkReadyInOrder(
	openParent bool,
	tmp *[]*priorityNodeRFC7540,
	f func(*priorityNodeRFC7540, bool) bool,
) bool {
...
func (ws *priorityWriteSchedulerRFC7540) CloseStream(streamID uint32) {
	...
	if ws.nodes[streamID].state != priorityNodeOpenRFC7540 {
		panic(
			fmt.Sprintf(
				"violation of WriteScheduler interface: stream %d already closed",
				streamID,
			),
		)
	}
...
func (ws *priorityWriteSchedulerRFC7540) addClosedOrIdleNode(
	list *[]*priorityNodeRFC7540,
	maxSize int,
	n *priorityNodeRFC7540,
) {
	if maxSize == 0 {
