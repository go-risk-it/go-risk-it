...
func (fr *Framer) ReadFrameHeader() (FrameHeader, error) {
	...
	if fh.Length > fr.maxReadSize {
		if fh == invalidHTTP1LookingFrameHeader() {
			return fh, fmt.Errorf(
				"http2: failed reading the frame payload: %w, note that the frame header looked like an HTTP/1.1 header",
				ErrFrameTooLarge,
			)
		}
...
func parsePriorityFrame(
	...
	if len(payload) != 5 {
		...
		return nil, connError{
			ErrCodeFrameSize,
			fmt.Sprintf("PRIORITY frame payload size was %d; want 5", len(payload)),
		}
	}
...
func parsePriorityUpdateFrame(
	...
	if len(payload) < 4 {
		...
		return nil, connError{
			ErrCodeFrameSize,
			fmt.Sprintf(
				"PRIORITY_UPDATE frame payload size was %d; want at least 4",
				len(payload),
			),
		}
	}
	...
	if streamID == 0 {
		...
		return nil, connError{
			ErrCodeProtocol,
			"PRIORITY_UPDATE frame with prioritized stream ID of zero",
		}
	}
...
// It will perform exactly one Write to the underlying Writer.
// It is the caller's responsibility to not call other Write methods concurrently.
func (f *Framer) WriteContinuation(
	streamID uint32,
	endHeaders bool,
...
func (mh *MetaHeadersFrame) rfc9218Priority(
	priorityAware bool,
) (p PriorityParam, priorityAwareAfter, hasIntermediary bool) {
	var s string
