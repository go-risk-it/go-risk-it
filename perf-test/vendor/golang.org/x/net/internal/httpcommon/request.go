...
var (
	ErrRequestHeaderListSize = errors.New(
			...
			if !validPseudoPath(path) {
				if req.URL.Opaque != "" {
					return res, fmt.Errorf(
						"invalid request :path %q from URL.Opaque = %q",
						orig,
						req.URL.Opaque,
					)
				} else {
...
func checkConnHeaders(h map[string][]string) error {
	...
	if vv := h["Transfer-Encoding"]; len(vv) > 0 &&
		(len(vv) > 1 || vv[0] != "" && vv[0] != "chunked") {
		return fmt.Errorf("invalid Transfer-Encoding request header: %q", vv)
	}
	if vv := h["Connection"]; len(vv) > 0 &&
		(len(vv) > 1 || vv[0] != "" && !asciiEqualFold(vv[0], "close") && !asciiEqualFold(vv[0], "keep-alive")) {
...
func NewServerRequest(rp ServerRequestParam) ServerRequestResult {
	...
	if strings.IndexByte(rp.Authority, '@') != -1 &&
