...
func Decode(encoded []byte) (rh *Histogram, err error) {
	...
	if Cookie != V2CompressedEncodingCookieBase {
		err = fmt.Errorf(
			"encoding not supported, only V2 is supported. got %d want %d",
			Cookie,
	...
	if lengthOfCompressedContents > decodeLengthOfCompressedContents {
		err = fmt.Errorf(
			"the compressed contents buffer is smaller than the lengthOfCompressedContents. got %d want %d",
			decodeLengthOfCompressedContents,
...
func (h *Histogram) encodeIntoByteBuffer() (*bytes.Buffer, error) {
	...
	err = binary.Write(
		toCompress,
		binary.BigEndian,
		h.getIntegerToDoubleValueConversionRatio(),
	) // 32-39
	if err != nil {
		return nil, err
	}
...
func decodeCompressedFormat(compressedContents []byte, headerSize int) (rh *Histogram, err error) {
	...
	cookie, PayloadLength, _, NumberOfSignificantValueDigits, LowestTrackableValue, HighestTrackableValue, _, err := decodeDeCompressedHeaderFormat(
		decompressedSlice[0:headerSize],
	)
	if err != nil {
		return
	}
	...
	if PayloadLength != actualPayloadLen {
		err = fmt.Errorf(
			"PayloadLength should have the same size of the actual payload. got %d want %d",
			actualPayloadLen,
			PayloadLength,
		)
		return
...
func decodeDeCompressedHeaderFormat(
	decoded []byte,
) (Cookie int32, PayloadLength int32, NormalizingIndexOffSet int32, NumberOfSignificantValueDigits int32, LowestTrackableValue int64, HighestTrackableValue int64, IntegerToDoubleConversionRatio float64, err error) {
