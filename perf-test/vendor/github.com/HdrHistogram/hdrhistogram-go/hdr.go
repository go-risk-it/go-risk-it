...
func New(
	...
	subBucketCountMagnitude := int32(
		math.Ceil(math.Log2(float64(largestValueWithSingleUnitResolution))),
	)
...
func (h *Histogram) nextNonEquivalentValue(v int64) int64 {
	bucketIdx := h.getBucketIndex(v)
	return h.lowestEquivalentValueGivenBucketIdx(
		v,
		bucketIdx,
	) + h.sizeOfEquivalentValueRangeGivenBucketIdx(
		v,
		bucketIdx,
	)
}
...
func (p *pIterator) next() bool {
	...
	for !done {
		...
		if p.countAtIdx != 0 && p.percentileToIteratorTo <= currentPercentile {
			...
			halfDistance := math.Trunc(
				math.Pow(2, math.Trunc(math.Log2(100.0/(100.0-p.percentileToIteratorTo)))+1),
			)
...
func (h *Histogram) PercentilesPrint(
	writer io.Writer,
	ticksPerHalfDistance int32,
	valueScale float64,
) (outputWriter io.Writer, err error) {
	outputWriter = writer
	...
	for _, slice := range dist {
		...
		_, err = fmt.Fprintf(
			outputWriter,
			"%12.3f %12f %12d %s\n",
			float64(slice.ValueAt)/valueScale,
			percentile,
			slice.Count,
			inverted_percentile_string,
		)
		if err != nil {
			return
		}
