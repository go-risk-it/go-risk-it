...
// since the epoch. For logging with absolute time stamps, the base time would remain zero ( default ).
// For logging with relative time stamps (time since a start point), the base time should be set with SetBaseTime(baseTime int64)
func (lw *HistogramLogWriter) OutputIntervalHistogramWithLogOptions(
	histogram *Histogram,
	logOptions *HistogramLogOptions,
) (err error) {
	tag := histogram.Tag()
	var match bool
	tagStr := ""
	if tag != "" {
		...
		if match {
			err = fmt.Errorf(
				"tag string cannot contain commas, spaces, or line breaks. used tag: %s",
				tag,
			)
			return
		}
...
func (lw *HistogramLogWriter) OutputLogFormatVersion() (err error) {
	return lw.OutputComment(
		fmt.Sprintf("[Histogram log format version %s]", HISTOGRAM_LOG_FORMAT_VERSION),
	)
}
