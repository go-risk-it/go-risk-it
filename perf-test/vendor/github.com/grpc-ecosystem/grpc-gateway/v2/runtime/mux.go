...
func WithForwardResponseOption(
	forwardResponseOption func(context.Context, http.ResponseWriter, proto.Message) error,
) ServeMuxOption {
	return func(serveMux *ServeMux) {
		serveMux.forwardResponseOptions = append(
			serveMux.forwardResponseOptions,
...
func WithIncomingHeaderMatcher(fn HeaderMatcherFunc) ServeMuxOption {
	for _, header := range fn.matchedMalformedHeaders() {
		grpclog.Warningf(
			"The configured forwarding filter would allow %q to be sent to the gRPC server, which will likely cause errors. See https://github.com/grpc/grpc-go/pull/4803#issuecomment-986093310 for more information.",
			header,
		)
	}
	...
	return func(s *ServeMux) {
		...
		_ = s.HandlePath(
			...
			func(w http.ResponseWriter, r *http.Request, _ map[string]string,
				...
				annotatedContext, err := AnnotateContext(
					r.Context(),
					s,
					r,
					grpc_health_v1.Health_Check_FullMethodName,
					WithHTTPPathPattern(endpointPath),
				)
				if err != nil {
				...
				resp, err := healthCheckClient.Check(
					annotatedContext,
...
func (s *ServeMux) isPathLengthFallback(r *http.Request) bool {
	return !s.disablePathLengthFallback && r.Method == "POST" &&
