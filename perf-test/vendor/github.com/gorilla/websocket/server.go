...
func (u *Upgrader) Upgrade(
	...
	if !tokenListContainsValue(r.Header, "Connection", "upgrade") {
		return u.returnError(
			...
			badHandshake+"'upgrade' token not found in 'Connection' header",
		)
	}
	...
	if !tokenListContainsValue(r.Header, "Upgrade", "websocket") {
		return u.returnError(
			...
			r,
			http.StatusBadRequest,
			badHandshake+"'websocket' token not found in 'Upgrade' header",
		)
	}
	...
	if r.Method != http.MethodGet {
		return u.returnError(
			...
			badHandshake+"request method is not GET",
		)
	}
	...
	if !isValidChallengeKey(challengeKey) {
		return u.returnError(
			...
			r,
			http.StatusBadRequest,
			"websocket: not a websocket handshake: 'Sec-WebSocket-Key' header must be Base64 encoded value of 16-byte in length",
		)
	}
