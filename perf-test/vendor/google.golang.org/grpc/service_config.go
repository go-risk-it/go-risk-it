...
var (
	...
	errEmptyServiceNonEmptyMethod = errors.New(
		"cannot combine empty 'service' and non-empty 'method'",
	)
)
...
func parseServiceConfig(js string, maxAttempts int) *serviceconfig.ParseResult {
	...
	if c == nil {
		...
		if err != nil {
			return &serviceconfig.ParseResult{
				Err: fmt.Errorf("unexpected error marshaling simple LB config: %w", err),
			}
		}
	...
	for _, m := range *rsc.MethodConfig {
		...
		for i, n := range *m.Name {
			...
			if err != nil {
				logger.Warningf(
					"grpc: error unmarshalling service config %s due to methodConfig[%d]: %v",
					js,
					i,
					err,
				)
	...
	if sc.retryThrottling != nil {
		if mt := sc.retryThrottling.MaxTokens; mt <= 0 || mt > 1000 {
			return &serviceconfig.ParseResult{
				Err: fmt.Errorf(
					"invalid retry throttling config: maxTokens (%v) out of range (0, 1000]",
					mt,
				),
			}
		}
		if tr := sc.retryThrottling.TokenRatio; tr <= 0 {
			return &serviceconfig.ParseResult{
				Err: fmt.Errorf(
					"invalid retry throttling config: tokenRatio (%v) may not be negative",
					tr,
				),
			}
		}
...
func convertRetryPolicy(
	jrp *jsonRetryPolicy,
	maxAttempts int,
) (p *internalserviceconfig.RetryPolicy, err error) {
	if jrp == nil {
		return nil, nil
	}
