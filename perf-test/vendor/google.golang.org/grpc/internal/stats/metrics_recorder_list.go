...
func (l *MetricsRecorderList) RecordInt64Count(
	handle *estats.Int64CountHandle,
	incr int64,
	labels ...string,
) {
...
// count associated with the provided handle.
func (l *MetricsRecorderList) RecordInt64UpDownCount(
	handle *estats.Int64UpDownCountHandle,
	incr int64,
	labels ...string,
) {
	verifyLabels(handle.Descriptor(), labels...)
...
// count associated with the provided handle.
func (l *MetricsRecorderList) RecordFloat64Count(
	handle *estats.Float64CountHandle,
	incr float64,
	labels ...string,
) {
	verifyLabels(handle.Descriptor(), labels...)
...
func (l *MetricsRecorderList) RecordInt64Histo(
...
// histo associated with the provided handle.
func (l *MetricsRecorderList) RecordFloat64Histo(
	handle *estats.Float64HistoHandle,
	incr float64,
	labels ...string,
) {
	verifyLabels(handle.Descriptor(), labels...)
...
func (l *MetricsRecorderList) RecordInt64Gauge(
	handle *estats.Int64GaugeHandle,
	incr int64,
	labels ...string,
) {
	verifyLabels(handle.Descriptor(), labels...)
...
// from all of them.
func (l *MetricsRecorderList) RegisterAsyncReporter(
	...
	for _, mr := range l.metricsRecorders {
		...
		unregisterFns = append(
			unregisterFns,
			mr.RegisterAsyncReporter(estats.AsyncMetricReporterFunc(wrappedCallback), metrics...),
		)
	}
...
// gauge associated with the provided handle.
func (w *asyncRecorderWrapper) RecordInt64AsyncGauge(
	handle *estats.Int64AsyncGaugeHandle,
	value int64,
	labels ...string,
) {
