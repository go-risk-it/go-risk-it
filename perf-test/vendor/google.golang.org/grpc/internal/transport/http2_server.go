...
var (
	...
	// the stream's state.
	ErrIllegalHeaderWrite = status.Error(
		codes.Internal,
		"transport: SendHeader called multiple times",
	)
	// ErrHeaderListSizeLimitViolation indicates that the header list size is larger
	// than the limit set by peer.
	ErrHeaderListSizeLimitViolation = status.Error(
		codes.Internal,
		"transport: trying to send header list size larger than the limit set by peer",
	)
)
...
func NewServerTransport(conn net.Conn, config *ServerConfig) (_ ServerTransport, err error) {
	...
	if err != nil {
		return nil, connectionErrorf(
			false,
			err,
			"transport: http2Server.HandleStreams failed to read initial settings frame: %v",
			err,
		)
	}
...
func (t *http2Server) operateHeaders(
	...
	for _, hf := range frame.Fields {
		...
		case "connection":
			if t.logger.V(logLevel) {
				t.logger.Infof(
					"Received a HEADERS frame with a :connection header which makes the request malformed, as per the HTTP/2 spec",
				)
			}
			protocolError = true
		default:
	...
	if len(mdata[":authority"]) > 1 || len(mdata["host"]) > 1 {
		errMsg := fmt.Sprintf(
			"num values of :authority: %v, num values of host: %v, both must only have 1 value as per HTTP/2 spec",
			len(mdata[":authority"]),
			len(mdata["host"]),
		)
		if t.logger.V(logLevel) {
		...
		t.writeEarlyAbort(
			streamID,
			s.contentSubtype,
			status.New(codes.Internal, errMsg),
	...
	if httpMethod != http.MethodPost {
		t.mu.Unlock()
		errMsg := fmt.Sprintf(
			"Received a HEADERS frame with :method %q which should be POST",
			httpMethod,
		)
		if t.logger.V(logLevel) {
			t.logger.Infof("Aborting the stream early: %v", errMsg)
	...
	if t.pingStrikes > maxPingStrikes {
		...
		t.controlBuf.put(
			&goAway{
				...
				debugData: []byte("too_many_pings"),
				closeConn: errors.New("got too many pings from the client"),
			},
		)
	}
}
...
func (t *http2Server) checkForHeaderListSize(hf []hpack.HeaderField) bool {
	...
	for _, f := range hf {
		if sz += int64(f.Size()); sz > int64(*t.maxSendHeaderListSize) {
			if t.logger.V(logLevel) {
				t.logger.Infof(
					"Header list size to send violates the maximum size (%d bytes) set by client",
...
// RST_STREAM instead.
func (t *http2Server) writeEarlyAbort(
	streamID uint32,
	...
	if p := istatus.RawStatusProto(stat); len(p.GetDetails()) > 0 {
		...
		if err == nil {
			hf = append(
				hf,
				hpack.HeaderField{
					Name:  grpcStatusDetailsBinHeader,
...
func (t *http2Server) writeHeaderLocked(s *ServerStream) error {
	...
	headerFields := make(
		...
		0,
		2,
	) // at least :status, content-type will be there if none else.
	...
	if s.sendCompress != "" {
		headerFields = append(
			headerFields,
			hpack.HeaderField{Name: "grpc-encoding", Value: s.sendCompress},
		)
	}
	...
	success, err := t.controlBuf.executeAndPut(
		func() bool { return t.checkForHeaderListSize(hf.hf) },
		hf,
	)
	if !success {
		if err != nil {
...
func (t *http2Server) writeStatus(s *ServerStream, st *status.Status) error {
	...
	headerFields := make(
		...
		0,
		2,
	) // grpc-status and grpc-message will be there if none else.
	...
	headerFields = append(
		headerFields,
		hpack.HeaderField{Name: "grpc-status", Value: strconv.Itoa(int(st.Code()))},
	)
	headerFields = append(
		headerFields,
		hpack.HeaderField{Name: "grpc-message", Value: encodeGrpcMessage(st.Message())},
	)
...
func (t *http2Server) finishStream(
	s *ServerStream,
	rst bool,
...
func (t *http2Server) closeStream(
	s *ServerStream,
	rst bool,
	rstCode http2.ErrCode,
	eosReceived bool,
