...
func dial(
	ctx context.Context,
	fn func(context.Context, string) (net.Conn, error),
...
func NewHTTP2Client(
	...
	if err != nil {
		if opts.FailOnNonTempDialError {
			return nil, connectionErrorf(
				isTemporary(err),
				err,
				"transport: error while dialing: %v",
				err,
			)
		}
	...
	if kp.Time != infinity {
		if err = isyscall.SetTCPUserTimeout(conn, kp.Timeout); err != nil {
			return nil, connectionErrorf(
				false,
				err,
				"transport: failed to set TCP_USER_TIMEOUT: %v",
				err,
			)
		}
	...
	if transportCreds != nil {
		...
		if err != nil {
			return nil, connectionErrorf(
				isTemporary(err),
				err,
				"transport: authentication handshake failed: %v",
				err,
			)
		}
	...
	if n != len(clientPreface) {
		err = connectionErrorf(
			true,
			nil,
			"transport: preface mismatch, wrote %d bytes; want %d",
			n,
			len(clientPreface),
		)
		return nil, err
	...
	if err != nil {
		err = connectionErrorf(
			true,
			err,
			"transport: failed to write initial settings frame: %v",
			err,
		)
		return nil, err
	}
...
func (t *http2Client) createHeaderFields(
	...
	if callHdr.PreviousAttempts > 0 {
		headerFields = append(
			...
			hpack.HeaderField{
				...
				Value: strconv.Itoa(callHdr.PreviousAttempts),
			},
		)
	}
	...
	if registeredCompressors != "" {
		headerFields = append(
			headerFields,
			hpack.HeaderField{Name: "grpc-accept-encoding", Value: registeredCompressors},
		)
	}
	...
	for k, v := range authData {
		headerFields = append(
			headerFields,
			hpack.HeaderField{Name: k, Value: encodeMetadataHeader(k, v)},
		)
	}
		...
		if err != nil {
			if st, ok := status.FromError(err); ok {
				...
				if istatus.IsRestrictedControlPlaneCode(st) {
					err = status.Errorf(
						codes.Internal,
						"transport: received per-RPC creds error with illegal status: %v",
						err,
					)
				}
				return nil, err
			...
			return nil, status.Errorf(
				...
				"transport: per-RPC creds failed due to error: %v",
				err,
			)
		}
		for k, v := range data {
...
func (t *http2Client) getCallAuthData(
	...
	if callCreds := callHdr.Creds; callCreds != nil {
		if callCreds.RequireTransportSecurity() {
			...
			if !t.isSecure ||
				credentials.CheckSecurityLevel(
					ri.AuthInfo,
	...
	if callHdr.Authority != "" {
		...
		if !ok {
			return nil, &NewStreamError{
				Err: status.Errorf(
					codes.Unavailable,
					"credentials type %q does not implement the AuthorityValidator interface, but authority override specified with CallAuthority call option",
					t.authInfo.AuthType(),
				),
			}
		}
		if err := auth.ValidateAuthority(callHdr.Authority); err != nil {
			return nil, &NewStreamError{
				Err: status.Errorf(
					codes.Unavailable,
					"failed to validate authority %q : %v",
					callHdr.Authority,
					err,
				),
			}
	...
	hdr := &headerFrame{
		...
		initStream: func(uint32) error {
			...
			if channelz.IsOn() {
				...
				t.channelz.SocketMetrics.LastLocalStreamCreatedTimestamp.Store(
					time.Now().UnixNano(),
				)
			}
...
func (t *http2Client) Close(err error) {
	...
	t.controlBuf.put(
		&goAway{
			...
			debugData: []byte("client transport shutdown"),
			closeConn: err,
		},
	)
	...
	case <-timer.C:
		t.logger.Infof(
			"Failed to write a GOAWAY frame as part of connection close after %s. Giving up and closing the transport.",
			goAwayLoopyWriterTimeout,
		)
	}
	t.cancel()
	t.conn.Close()
...
func (t *http2Client) handleData(f *parsedDataFrame) {
	...
	if f.StreamEnded() {
		t.closeStream(
			...
			false,
			http2.ErrCodeNo,
			status.New(codes.Internal, "server closed the stream without sending trailers"),
			nil,
			true,
		)
	}
...
func (t *http2Client) handleRSTStream(f *http2.RSTStreamFrame) {
	...
	if !ok {
		if t.logger.V(logLevel) {
			t.logger.Infof(
				"Received a RST_STREAM frame with code %q, but found no mapped gRPC status",
				f.ErrCode,
			)
		}
...
func (t *http2Client) handleGoAway(f *http2.GoAwayFrame) error {
	...
	if id > 0 && id%2 == 0 {
		...
		return connectionErrorf(
			true,
			nil,
			"received goaway with non-zero even-numbered stream id: %v",
			id,
		)
	}
	...
	case <-t.goAway: // t.goAway has been closed (i.e.,multiple GoAways).
		...
		if id > t.prevGoAwayID {
			...
			return connectionErrorf(
				true,
				nil,
				"received goaway with stream id: %v, which exceeds stream id of previous goaway: %v",
				id,
				t.prevGoAwayID,
			)
		}
	...
	for _, hf := range frame.Fields {
		...
		case "content-type":
			if _, validContentType := grpcutil.ContentSubtype(hf.Value); !validContentType {
				contentTypeErr = fmt.Sprintf(
					"transport: received unexpected content-type %q",
					hf.Value,
				)
				break
			}
...
func (t *http2Client) readServerPreface() error {
	...
	if !ok {
		return connectionErrorf(
			true,
			nil,
			"initial http2 frame from server is not a settings frame: %T",
			frame,
		)
	}
	...
	for {
		...
		case <-timer.C:
			...
			if lastRead > prevNano {
				...
				timer.Reset(
					time.Duration(lastRead) + t.kp.Time - time.Duration(time.Now().UnixNano()),
				)
				prevNano = lastRead
				continue
			}
