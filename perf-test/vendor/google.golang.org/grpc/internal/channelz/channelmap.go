...
func (c *channelMap) traceEvent(id int64, desc *TraceEvent) {
	...
	childTC.getChannelTrace().
		append(&traceEvent{Desc: desc.Desc, Severity: desc.Severity, Timestamp: time.Now()})
	if desc.Parent != nil {
...
func (d *dummyEntry) addChild(id int64, e entry) {
	...
	logger.Infof(
		"attempt to add child of type %T with id %d to a parent (id=%d) that doesn't currently exist",
		e,
		id,
		d.idNotFound,
	)
...
func (d *dummyEntry) deleteChild(id int64) {
	...
	logger.Infof(
		"attempt to delete child with id %d from a parent (id=%d) that doesn't currently exist",
		id,
		d.idNotFound,
	)
}
...
func (d *dummyEntry) triggerDelete() {
	logger.Warningf(
		"attempt to delete an entry (id=%d) that doesn't currently exist",
		d.idNotFound,
	)
}
