...
func New(
	...
	targetResolverBuilder resolver.Builder,
	...
	if target.URL.Scheme == "dns" && !targetResolutionEnabled &&
		envconfig.EnableDefaultPortForProxyTarget {
	...
	if r.targetResolver, err = targetResolverBuilder.Build(target, wcc, opts); err != nil {
		return nil, fmt.Errorf(
			"delegating_resolver: unable to build the resolver for target %s: %v",
			target,
			err,
		)
	}
...
func (r *delegatingResolver) updateTargetResolverState(state resolver.State) error {
	...
	if len(r.proxyAddrs) == 0 {
		go func() {
			...
			if err != nil {
				r.cc.ReportError(
					fmt.Errorf("delegating_resolver: unable to build the proxy resolver: %v", err),
				)
				return
			}
...
func (wcc *wrappingClientConn) ParseServiceConfig(
