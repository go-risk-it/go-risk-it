...
func chainUnaryInterceptors(interceptors []UnaryServerInterceptor) UnaryServerInterceptor {
	return func(ctx context.Context, req any, info *UnaryServerInfo, handler UnaryHandler) (any, error) {
		return interceptors[0](
			...
			req,
			info,
			getChainUnaryHandler(interceptors, 0, info, handler),
		)
	}
}
...
func (s *Server) processUnaryRPC(
	...
	d, err := recvAndDecompress(
		&parser{r: stream, bufferPool: s.opts.bufferPool},
		stream,
		dc,
		s.opts.maxReceiveMessageSize,
		payInfo,
		decomp,
		true,
	)
...
func chainStreamServerInterceptors(s *Server) {
	...
	if s.opts.streamInt != nil {
		interceptors = append(
			[]StreamServerInterceptor{s.opts.streamInt},
			s.opts.chainStreamInts...)
	}
...
func chainStreamInterceptors(interceptors []StreamServerInterceptor) StreamServerInterceptor {
	return func(srv any, ss ServerStream, info *StreamServerInfo, handler StreamHandler) error {
		return interceptors[0](
			...
			ss,
			info,
			getChainStreamHandler(interceptors, 0, info, handler),
		)
	}
}
...
func (s *Server) processStreamingRPC(
	ctx context.Context,
	stream *transport.ServerStream,
	info *serviceInfo,
	sd *StreamDesc,
...
func (s *Server) getCodec(contentSubtype string) baseCodec {
	...
	if codec == nil {
		logger.Warningf(
			"Unsupported codec %q. Defaulting to %q for now. This will start to fail in future releases.",
			contentSubtype,
			proto.Name,
		)
		return getCodec(proto.Name)
...
func SetHeader(ctx context.Context, md metadata.MD) error {
	...
	if stream == nil {
		return status.Errorf(
			codes.Internal,
			"grpc: failed to fetch the stream from the context %v",
			ctx,
		)
	}
