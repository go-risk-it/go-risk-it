	...
	if err != nil {
		if st, ok := status.FromError(err); ok {
			...
			if istatus.IsRestrictedControlPlaneCode(st) {
				err = status.Errorf(
					codes.Internal,
					"config selector returned illegal status: %v",
					err,
				)
			}
			return nil, err
...
func newClientStreamWithParams(
	...
	cc *ClientConn,
	method string,
	mc *serviceconfig.MethodConfig,
	onCommit, doneFunc func(),
	nameResolutionDelayed bool,
	opts ...CallOption,
) (_ iresolver.ClientStream, err error) {
	...
	callInfo.maxSendMessageSize = getMaxSize(
		mc.MaxReqSize,
		callInfo.maxSendMessageSize,
		defaultClientMaxSendMessageSize,
	)
	callInfo.maxReceiveMessageSize = getMaxSize(
		mc.MaxRespSize,
		callInfo.maxReceiveMessageSize,
...
func (cs *clientStream) newAttemptLocked(isTransparent bool) (*csAttempt, error) {
	...
	ctx := newContextWithRPCInfo(
		cs.ctx,
		cs.callInfo.failFast,
		cs.callInfo.codec,
		cs.compressorV0,
		cs.compressorV1,
	)
		...
		if len(sps) == 1 {
			...
			if pushback, e = strconv.Atoi(sps[0]); e != nil || pushback < 0 {
				channelz.Infof(
					logger,
					cs.cc.channelz,
					"Server retry pushback specified to abort (%q).",
					sps[0],
				)
...
func (cs *clientStream) SendMsg(m any) (err error) {
	...
	hdr, data, payload, pf, err := prepareMsg(
		m,
		cs.codec,
		cs.compressorV0,
		cs.compressorV1,
		cs.cc.dopts.copts.BufferPool,
	)
	if err != nil {
		return err
	}
	...
	if payloadLen > *cs.callInfo.maxSendMessageSize {
		return status.Errorf(
			...
			payloadLen,
			*cs.callInfo.maxSendMessageSize,
		)
	}
...
func (a *csAttempt) sendMsg(
	m any,
	hdr []byte,
	payld mem.BufferSlice,
...
func (a *csAttempt) recvMsg(m any, payInfo *payloadInfo) (err error) {
	...
	if !a.decompressorSet {
		...
		if ct := a.transportStream.RecvCompress(); ct != "" && ct != encoding.Identity {
			...
			if !acceptedCompressorAllows(cs.callInfo.acceptedResponseCompressors, ct) {
				return status.Errorf(
					codes.Internal,
					"grpc: peer compressed the response with %q which is not allowed by AcceptCompressors",
					ct,
				)
			}
		} else {
	...
	if err := recv(&a.parser, cs.codec, a.transportStream, a.decompressorV0, m, *cs.callInfo.maxReceiveMessageSize, payInfo, a.decompressorV1, false); err != nil {
		if err == io.EOF {
			...
			if !cs.desc.ServerStreams && !cs.receivedFirstMsg {
				return status.Error(
					codes.Internal,
					"cardinality violation: received no response message from non-server-streaming RPC",
				)
			}
	...
	return status.Error(
		codes.Internal,
		"cardinality violation: expected <EOF> for non server-streaming RPCs, but received another message",
	)
}
...
func newNonRetryClientStream(
	...
	c.maxReceiveMessageSize = getMaxSize(
		nil,
		c.maxReceiveMessageSize,
		defaultClientMaxReceiveMessageSize,
	)
...
func (as *addrConnStream) RecvMsg(m any) (err error) {
	...
	if err := recv(&as.parser, as.codec, as.transportStream, as.decompressorV0, m, *as.callInfo.maxReceiveMessageSize, nil, as.decompressorV1, false); err == io.EOF {
		return as.transportStream.Status().
			Err()
		// non-server streaming Recv returns nil on success
	} else if err != nil {
		return toRPCErr(err)
	}
	return status.Error(
...
func (ss *serverStream) SendMsg(m any) (err error) {
	...
	if ss.statsHandler != nil {
		ss.statsHandler.HandleRPC(
			ss.s.Context(),
			outPayload(false, m, dataLen, payloadLen, time.Now()),
		)
	}
	return nil
}
...
func (ss *serverStream) RecvMsg(m any) (err error) {
	...
	if err := recv(&ss.p, ss.codec, ss.s, ss.decompressorV0, m, ss.maxReceiveMessageSize, payInfo, ss.decompressorV1, true); err != nil {
		if err == io.EOF {
			...
			if !ss.desc.ClientStreams && !ss.recvFirstMsg {
				return status.Error(
					codes.Internal,
					"cardinality violation: received no request message from non-client-streaming RPC",
				)
			}
			return err
		}
	...
	return status.Error(
		codes.Internal,
		"cardinality violation: received multiple request messages for non-client-streaming RPC",
	)
}
...
func prepareMsg(
	...
	pool mem.BufferPool,
) (hdr []byte, data, payload mem.BufferSlice, pf payloadFormat, err error) {
