...
func (c *tlsCreds) ClientHandshake(
	...
	authority string,
	rawConn net.Conn,
) (_ net.Conn, _ AuthInfo, err error) {
	...
	if np == "" {
		if envconfig.EnforceALPNEnabled {
			...
			return nil, nil, fmt.Errorf(
				...
				alpnFailureHelpMessage,
			)
		}
		logger.Warningf(
			"Allowing TLS connection to server %q with ALPN disabled. TLS connections to servers with ALPN disabled will be disallowed in future grpc-go releases",
			cfg.ServerName,
		)
	}
	tlsInfo := TLSInfo{
