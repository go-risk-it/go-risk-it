...
var (
	...
	errNoTransportSecurity = errors.New(
...
func (dcs *defaultConfigSelector) SelectConfig(
	rpcInfo iresolver.RPCInfo,
) (*iresolver.RPCConfig, error) {
...
func DialContext(
	...
	opts = append(
		[]DialOption{withDefaultScheme("passthrough"), WithLocalDNSResolution()},
		opts...)
...
func (cc *ClientConn) validateTransportCredentials() error {
	...
	if cc.dopts.copts.CredsBundle != nil &&
		cc.dopts.copts.CredsBundle.TransportCredentials() == nil {
...
func newConnectivityStateManager(
	ctx context.Context,
...
func (cc *ClientConn) WaitForStateChange(
	ctx context.Context,
	sourceState connectivity.State,
) bool {
...
func (cc *ClientConn) maybeApplyDefaultServiceConfig() {
	...
	if cc.dopts.defaultServiceConfig != nil {
		cc.applyServiceConfigAndBalancer(
			...
			&defaultConfigSelector{cc.dopts.defaultServiceConfig},
		)
	} else {
...
func (cc *ClientConn) updateResolverStateAndUnlock(s resolver.State, err error) error {
	...
	if cc.dopts.disableServiceConfig {
		channelz.Infof(
			logger,
			cc.channelz,
			"ignoring service config from resolver (%v) and applying the default because service config is disabled",
			s.ServiceConfig,
		)
		cc.maybeApplyDefaultServiceConfig()
	...
	uccsErr := bw.updateClientConnState(
		&balancer.ClientConnState{ResolverState: s, BalancerConfig: balCfg},
	)
	if ret == nil {
...
func (cc *ClientConn) newAddrConnLocked(
...
func equalAddressesIgnoringBalAttributes(a, b []resolver.Address) bool {
	return slices.EqualFunc(
		a,
		b,
		func(a, b resolver.Address) bool { return equalAddressIgnoringBalAttributes(&a, &b) },
	)
}
...
func (ac *addrConn) updateAddrs(addrs []resolver.Address) {
	...
	channelz.Infof(
		logger,
		ac.channelz,
		"addrConn: updateAddrs addrs (%d of %d): %v",
		limit,
		len(addrs),
		addrs[:limit],
	)
...
func (ac *addrConn) updateConnectivityState(s connectivity.State, lastErr error) {
	...
	if ac.state == connectivity.Ready ||
		(ac.state == connectivity.Connecting && s == connectivity.Idle) {
		disconnectionsMetric.Record(
			ac.cc.metricsRecorderList,
			1,
			ac.cc.target,
		...
		openConnectionsMetric.Record(
			ac.cc.metricsRecorderList,
			-1,
			ac.cc.target,
			ac.backendServiceLabel,
			ac.securityLevelLocked(),
			ac.localityLabel,
		)
	}
	ac.state = s
...
func (ac *addrConn) resetTransportAndUnlock() {
	...
	if err := ac.tryAllAddrs(acCtx, addrs, connectDeadline); err != nil {
		if !errors.Is(err, context.Canceled) {
			connectionAttemptsFailedMetric.Record(
				ac.cc.metricsRecorderList,
				1,
				ac.cc.target,
				ac.backendServiceLabel,
	...
	ac.mu.Lock()
	connectionAttemptsSucceededMetric.Record(
		ac.cc.metricsRecorderList,
		1,
		ac.cc.target,
		ac.backendServiceLabel,
...
func (ac *addrConn) tryAllAddrs(
	...
	for _, addr := range addrs {
		...
		channelz.Infof(
			logger,
			ac.channelz,
			"Subchannel picks a new address %q to connect",
			addr.Addr,
		)
...
func (ac *addrConn) createTransport(
	...
	if err != nil {
		...
		channelz.Warningf(
			logger,
			ac.channelz,
			"grpc: addrConn.createTransport failed to connect to %s. Err: %v",
			addr,
			err,
		)
		return err
...
func (ac *addrConn) startHealthCheck(ctx context.Context) {
	...
	if healthCheckFunc == nil {
		...
		channelz.Error(
			logger,
			ac.channelz,
			"Health check is requested but health check function is not set.",
		)
		return
	}
	...
	newStream := func(method string) (any, error) {
		...
		if ac.transport != currentTr {
			...
			return nil, status.Error(
				codes.Canceled,
				"the provided transport is no longer valid to use",
			)
		}
		ac.mu.Unlock()
		return newNonRetryClientStream(
			ctx,
			&StreamDesc{ServerStreams: true},
			method,
			currentTr,
			ac,
		)
		...
		if err != nil {
			if status.Code(err) == codes.Unimplemented {
				channelz.Error(
					logger,
					ac.channelz,
					"Subchannel health check is unimplemented at server side, thus health check is disabled",
				)
			} else {
...
func (cc *ClientConn) initAuthority() error {
	...
	if (authorityFromCreds != "" && authorityFromDialOption != "") &&
		authorityFromCreds != authorityFromDialOption {
		return fmt.Errorf(
