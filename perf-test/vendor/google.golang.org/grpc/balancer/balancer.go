...
func Register(b Builder) {
	...
	if name != b.Name() {
		...
		logger.Warningf(
			"Balancer registered with name %q. grpc-go will be switching to case sensitive balancer registries soon",
			b.Name(),
		)
	}
	m[name] = b
...
func Get(name string) Builder {
	if strings.ToLower(name) != name {
		...
		// remove this warning and update the docstrings for Register and Get.
		logger.Warningf(
			"Balancer retrieved for name %q. grpc-go will be switching to case sensitive balancer registries soon",
			name,
		)
	}
