...
func (pickfirstBuilder) ParseConfig(
	js json.RawMessage,
...
func (b *pickfirstBalancer) resolverErrorLocked(err error) {
	...
	if b.state != connectivity.TransientFailure && b.addressList.size() > 0 {
		if b.logger.V(2) {
			b.logger.Infof(
				"Ignoring resolver error because balancer is using a previous good update.",
			)
		}
		return
	}
...
func (b *pickfirstBalancer) UpdateClientConnState(state balancer.ClientConnState) error {
	...
	b.healthCheckingEnabled = state.ResolverState.Attributes.Value(
		enableHealthListenerKeyType{},
	) != nil
	...
	if b.logger.V(2) {
		b.logger.Infof(
			"Received new config %s, resolver state %s",
			pretty.ToJSON(cfg),
			pretty.ToJSON(state.ResolverState),
		)
	}
	...
	isPrevRawConnectivityStateReady := found &&
		prevSCData.rawConnectivityState == connectivity.Ready
	...
	if isPrevRawConnectivityStateReady || b.state == connectivity.Connecting ||
		prevAddrsCount == 0 {
...
// creating SubConns.
func (b *pickfirstBalancer) UpdateSubConnState(
	subConn balancer.SubConn,
	...
	for valid := true; valid; valid = b.addressList.increment() {
		...
		if !ok {
			...
			if err != nil {
				...
				if b.logger.V(2) {
					b.logger.Infof(
						"Failed to create a subConn for address %v: %v",
						curAddr.String(),
						err,
					)
				}
		...
		default:
			b.logger.Errorf(
				"SubConn with unexpected state %v present in SubConns map.",
				sd.rawConnectivityState,
			)
			return
...
func (b *pickfirstBalancer) scheduleNextConnectionLocked() {
	...
	closeFn := internal.TimeAfterFunc(connectionDelayInterval, func() {
		...
		if b.logger.V(2) {
			b.logger.Infof(
				"Happy Eyeballs timer expired while waiting for connection to %q.",
				curAddr.Addr,
			)
		}
...
func (b *pickfirstBalancer) updateSubConnState(sd *scData, newState balancer.SubConnState) {
	...
	if newState.ConnectivityState == connectivity.Ready {
		...
		if !b.addressList.seekTo(sd.addr) {
			...
			b.logger.Errorf(
				"Address %q not found address list in %v",
				sd.addr,
				b.addressList.addresses,
			)
			return
		}
		if !b.healthCheckingEnabled {
			if b.logger.V(2) {
				b.logger.Infof(
					"SubConn %p reported connectivity state READY and the health listener is disabled. Transitioning SubConn to READY.",
					sd.subConn,
				)
			}
		...
		if b.logger.V(2) {
			b.logger.Infof(
				"SubConn %p reported connectivity state READY. Registering health listener.",
				sd.subConn,
			)
		}
	...
	if oldState == connectivity.Ready ||
		(oldState == connectivity.Connecting && newState.ConnectivityState == connectivity.Idle) {
	...
	if b.firstPass {
		...
		case connectivity.TransientFailure:
			...
			if curAddr := b.addressList.currentAddress(); equalAddressIgnoringBalAttributes(
				&curAddr,
				&sd.addr,
			) {
				b.cancelConnectionTimer()
...
func (b *pickfirstBalancer) updateSubConnHealthState(sd *scData, state balancer.SubConnState) {
	...
	case connectivity.TransientFailure:
		b.updateBalancerState(balancer.State{
			ConnectivityState: connectivity.TransientFailure,
			Picker: &picker{
				err: fmt.Errorf("pickfirst: health check failure: %v", state.ConnectionError),
			},
		})
	case connectivity.Connecting:
		b.updateBalancerState(balancer.State{
