...
func (ccb *ccBalancerWrapper) NewSubConn(
	...
	if err != nil {
		channelz.Warningf(
			logger,
			ccb.cc.channelz,
			"acBalancerWrapper: NewSubConn: failed to newAddrConn: %v",
			err,
		)
		return nil, err
	}
...
func (acbw *acBalancerWrapper) NewStream(
	ctx context.Context,
	desc *StreamDesc,
	method string,
	opts ...CallOption,
...
// errSubConnNotReady.
func (acbw *acBalancerWrapper) Invoke(
	ctx context.Context,
	method string,
	args any,
	reply any,
...
func (acbw *acBalancerWrapper) GetOrBuildProducer(
	pb balancer.ProducerBuilder,
...
func (acbw *acBalancerWrapper) healthListenerRegFn() func(context.Context, func(balancer.SubConnState)) func() {
	...
	if regHealthLisFn == nil {
		...
		channelz.Error(
			logger,
			acbw.ac.channelz,
			"Health check is requested but health package is not imported.",
		)
		return noOpRegisterHealthListenerFn
	}
