	...
	for _, name := range names {
		...
		if !grpcutil.IsCompressorNameRegistered(name) {
			return nil, status.Errorf(
				...
				"grpc: compressor %q is not registered",
				name,
			)
		}
...
func encode(c baseCodec, msg any) (mem.BufferSlice, error) {
	...
	if bufSize := uint(b.Len()); bufSize > math.MaxUint32 {
		...
		return nil, status.Errorf(
			codes.ResourceExhausted,
			"grpc: message too large (%d bytes)",
			bufSize,
		)
	}
	return b, nil
...
func compress(
	...
	pool mem.BufferPool,
) (mem.BufferSlice, payloadFormat, error) {
...
func msgHeader(
	data, compData mem.BufferSlice,
	pf payloadFormat,
) (hdr []byte, payload mem.BufferSlice) {
...
func decompress(
	...
	if dc != nil {
		...
		if err != nil {
			return nil, status.Errorf(
				codes.Internal,
				"grpc: failed to decompress the received message: %v",
				err,
			)
		}
	...
	if compressor != nil {
		...
		if err != nil {
			return nil, status.Errorf(
				codes.Internal,
				"grpc: failed to decompress the message: %v",
				err,
			)
		}
		...
		if err != nil {
			...
			return nil, status.Errorf(
				codes.Internal,
				"grpc: failed to read decompressed data: %v",
				err,
			)
		}
	...
	return nil, status.Errorf(
		codes.Internal,
		"grpc: no decompressor available for compressed payload",
	)
}
...
func recv(
	...
	if err := c.Unmarshal(data, m); err != nil {
		return status.Errorf(
			codes.Internal,
			"grpc: failed to unmarshal the received message: %v",
			err,
		)
	}
...
func setCallInfoCodec(c *callInfo) error {
	...
	if c.codec == nil {
		return status.Errorf(
			codes.Internal,
			"no codec registered for content-subtype %s",
			c.contentSubtype,
		)
	}
	return nil
