...
func aberrantLoadMessageDescReentrant(
	t reflect.Type,
	name protoreflect.FullName,
) protoreflect.MessageDescriptor {
	...
	if fn, ok := t.MethodByName("ExtensionRangeArray"); ok {
		...
		for i := 0; i < vs.Len(); i++ {
			v := vs.Index(i)
			md.L2.ExtensionRanges.List = append(
				md.L2.ExtensionRanges.List,
				[2]protoreflect.FieldNumber{
...
func aberrantAppendField(md *filedesc.Message, goType reflect.Type, tag, tagKey, tagVal string) {
	...
	if fd.L1.EditionFeatures.IsPacked {
		fd.L1.Options = func() protoreflect.ProtoMessage {
			...
			if fd.L1.EditionFeatures.IsPacked {
				opts.Set(
					opts.Descriptor().Fields().ByName("packed"),
					protoreflect.ValueOfBool(fd.L1.EditionFeatures.IsPacked),
				)
			}
