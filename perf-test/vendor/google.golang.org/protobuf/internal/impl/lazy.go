...
func (mi *MessageInfo) lazyUnmarshal(p pointer, num protoreflect.FieldNumber) {
	...
	if !found && multipleEntries == nil {
		panic(
			fmt.Sprintf("lazyUnmarshal: can't find field data for %v.%v", mi.Desc.FullName(), num),
		)
	}
	...
	if multipleEntries != nil {
		for _, entry := range multipleEntries {
			mi.unmarshalField(
				lazy.Buffer()[entry.Start:entry.End],
				fp,
				f,
				lazy,
				lazy.UnmarshalFlags(),
...
func (mi *MessageInfo) unmarshalField(
	b []byte,
	p pointer,
	f *coderFieldInfo,
	lazyInfo *protolazy.XXX_lazyUnmarshalInfo,
	flags piface.UnmarshalInputFlags,
) error {
	...
	for len(b) > 0 {
		...
		if n := tag >> 3; n < uint64(protowire.MinValidNumber) ||
			n > uint64(protowire.MaxValidNumber) {
			return errors.New("invalid wire data")
		} else {
...
func (mi *MessageInfo) skipField(
	b []byte,
	f *coderFieldInfo,
	wtyp protowire.Type,
	opts unmarshalOptions,
) (out unmarshalOutput, _ ValidationStatus) {
	fmi := f.validation.mi
	if fmi == nil {
...
func (mi *MessageInfo) unmarshalPointerLazy(
	b []byte,
	p pointer,
	groupTag protowire.Number,
	opts unmarshalOptions,
