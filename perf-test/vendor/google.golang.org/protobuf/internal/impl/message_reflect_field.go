...
func fieldInfoForOneof(
	...
	if ft.Kind() != reflect.Interface {
		panic(
			fmt.Sprintf(
				"field %v has invalid type: got %v, want interface kind",
				fd.FullName(),
				ft,
			),
		)
	...
	if ot.Kind() != reflect.Struct {
		panic(
			fmt.Sprintf("field %v has invalid type: got %v, want struct kind", fd.FullName(), ot),
		)
	}
	if !reflect.PtrTo(ot).Implements(ft) {
		panic(
			fmt.Sprintf(
				"field %v has invalid type: %v does not implement %v",
				fd.FullName(),
				ot,
				ft,
			),
	...
	return fieldInfo{
		...
		mutable: func(p pointer) protoreflect.Value {
			if !isMessage {
				panic(
					fmt.Sprintf(
						"field %v with invalid Mutable call on field with non-composite type",
						fd.FullName(),
					),
				)
			}
...
func fieldInfoForMap(
	...
	return fieldInfo{
		...
		set: func(p pointer, v protoreflect.Value) {
			...
			if pv.IsNil() {
				panic(
					fmt.Sprintf("map field %v cannot be set with read-only value", fd.FullName()),
				)
			}
			rv.Set(pv)
		},
...
func fieldInfoForList(
	...
	return fieldInfo{
		...
		set: func(p pointer, v protoreflect.Value) {
			...
			if pv.IsNil() {
				panic(
					fmt.Sprintf("list field %v cannot be set with read-only value", fd.FullName()),
				)
			}
			rv.Set(pv.Elem())
		},
...
func fieldInfoForScalar(
	...
	return fieldInfo{
		...
		has: func(p pointer) bool {
			...
			default:
				panic(
					fmt.Sprintf("field %v has invalid type: %v", fd.FullName(), rv.Type()),
				) // should never happen
			}
		},
		clear: func(p pointer) {
