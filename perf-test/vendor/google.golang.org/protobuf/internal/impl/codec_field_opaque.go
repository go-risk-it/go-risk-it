...
func makeOpaqueMessageFieldCoder(
	fd protoreflect.FieldDescriptor,
	ft reflect.Type,
) (*MessageInfo, pointerCoderFuncs) {
...
func makeOpaqueRepeatedMessageFieldCoder(
	fd protoreflect.FieldDescriptor,
	ft reflect.Type,
) (*MessageInfo, pointerCoderFuncs) {
	if ft.Kind() != reflect.Ptr || ft.Elem().Kind() != reflect.Slice {
		panic(
			fmt.Sprintf(
				"invalid field: %v: unsupported type for opaque repeated message: %v",
				fd.FullName(),
				ft,
			),
		)
...
func consumeOpaqueMessageSlice(
	b []byte,
	p pointer,
	wtyp protowire.Type,
	f *coderFieldInfo,
