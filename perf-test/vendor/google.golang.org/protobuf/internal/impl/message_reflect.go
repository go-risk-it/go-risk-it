...
func (mi *MessageInfo) checkField(
	fd protoreflect.FieldDescriptor,
) (*fieldInfo, protoreflect.ExtensionTypeDescriptor) {
	var fi *fieldInfo
	...
	if fd.IsExtension() {
		if got, want := fd.ContainingMessage().FullName(), mi.Desc.FullName(); got != want {
			...
			panic(
				fmt.Sprintf(
					"extension %v has mismatching containing message: got %v, want %v",
					fd.FullName(),
					got,
					want,
				),
		...
		if !mi.Desc.ExtensionRanges().Has(fd.Number()) {
			panic(
				fmt.Sprintf(
					"extension %v extends %v outside the extension range",
					fd.FullName(),
					mi.Desc.FullName(),
				),
			)
		...
		if !ok {
			panic(
				fmt.Sprintf(
					"extension %v does not implement protoreflect.ExtensionTypeDescriptor",
					fd.FullName(),
				),
			)
		}
