...
func (mi *MessageInfo) checkInitialized(
	in protoiface.CheckInitializedInput,
) (protoiface.CheckInitializedOutput, error) {
	var p pointer
	...
	for _, f := range mi.orderedCoderFields {
		...
		if f.presenceIndex != noPresence {
			if !presence.Present(f.presenceIndex) {
				if f.isRequired {
					return errors.RequiredNotSet(
						string(mi.Desc.Fields().ByNumber(f.num).FullName()),
					)
				}
				continue
			}
