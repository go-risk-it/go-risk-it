...
func (lazy *XXX_lazyUnmarshalInfo) FindFieldInProto(
	fieldNum uint32,
) (start, end uint32, found, multipleContiguous bool, multipleEntries []IndexEntry) {
	...
	if index == nil {
		...
		if err != nil {
			panic(
				fmt.Sprintf(
					"findFieldInfo: error building index when looking for field %d: %v",
					fieldNum,
					err,
				),
			)
...
func lookupField(
	indexp *[]IndexEntry,
	fieldNum uint32,
) (start, end uint32, found bool, multipleContiguous bool, multipleEntries []IndexEntry) {
	...
	for i, entry := range index {
		if fieldNum == entry.FieldNum {
			if i < len(index)-1 && entry.FieldNum == index[i+1].FieldNum {
				...
				multiple[0] = IndexEntry{
					fieldNum,
					entry.Start,
					entry.End,
					entry.MultipleContiguous,
				}
				i++
