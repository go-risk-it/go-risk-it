...
func (e encoder) marshalAny(m protoreflect.Message) error {
	...
	if err != nil {
		return errors.New(
			"%s: unable to unmarshal %q: %v",
			genid.Any_message_fullname,
			typeURL,
			err,
		)
	}
...
func (e encoder) marshalDuration(m protoreflect.Message) error {
	...
	if (secs > 0 && nanos < 0) || (secs < 0 && nanos > 0) {
		return errors.New(
			"%s: signs of seconds and nanos do not match",
			genid.Duration_message_fullname,
		)
	}
...
func (d decoder) unmarshalTimestamp(m protoreflect.Message) error {
	...
	if err != nil {
		return d.newError(
			tok.Pos(),
			"invalid %v value %v",
			genid.Timestamp_message_fullname,
			tok.RawString(),
		)
	}
	// Validate seconds.
...
func (e encoder) marshalFieldMask(m protoreflect.Message) error {
	...
	for i := 0; i < list.Len(); i++ {
		...
		if !protoreflect.FullName(s).IsValid() {
			return errors.New(
				"%s contains invalid path: %q",
				genid.FieldMask_Paths_field_fullname,
				s,
			)
		}
		...
		if s != strs.JSONSnakeCase(cc) {
			return errors.New(
				"%s contains irreversible value %q",
				genid.FieldMask_Paths_field_fullname,
				s,
			)
		}
