...
func TestConfig_Defaults(t *testing.T) {
	...
	assert.Equal(t, "", cfg.Server.AnonKey)
	assert.Equal(t, "map.json", cfg.Server.MapFile)
...
func parseTestFlags(t *testing.T, args ...string) *Config {
	...
	if savedKey != "" {
		os.Unsetenv("ANON_KEY")                               //nolint:errcheck
		t.Cleanup(func() { os.Setenv("ANON_KEY", savedKey) }) //nolint:errcheck
	}
