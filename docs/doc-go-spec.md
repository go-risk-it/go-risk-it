...
| `game-domain` | Game-scoped context types (game/ctx) | Imports only `kernel/` |
| `game-support` | Cross-cutting game infrastructure (config, headlines, snapshot) | May import `data/`, `events-domain/`, `game-domain/`, `kernel/`, `logic/` |
| `kernel` | Shared infrastructure (config, ctx, data, errors, bus, metrics) | Never imports `game/` or `lobby/` |
| `lobby-domain` | Lobby-scoped context types (lobby/ctx) | Imports only `kernel/` |
...
| `game/internal/logic/move/attack` | logic | 5 |
