...
- **Living Architecture** -- Every package has a [`doc.go`](docs/doc-go-spec.md) with structured sections (`# Layer`, `# Key Types`, `# Dependencies`). CI validates layer claims against the actual import graph. [28 architecture rules](internal/arch_test.go) enforce boundaries at test time -- import constraints, quality ratchets, and documentation coverage.
...
- **Type-Safe Move Pipeline** -- Generic `Service[T, R]` interface with compile-time enforcement across 5 move types. The [orchestration pipeline](internal/game/logic/move/orchestration/doc.go) runs validate, perform, log, check mission, and advance in a single transaction.
...
[28 rules](internal/arch_test.go) run as standard Go tests, validating layer boundaries (logic never imports web), module isolation (game and lobby are fully independent), import guards (no stdlib `log`, no old `math/rand`), package quality ratchets (export ceilings, fan-out limits, file count limits), and documentation coverage (doc.go existence and section completeness).
...
│   │   ├── messaging/          # Package messaging defines the WebSocket message types sent to game clients.
│   │   └── rest/
...
│   ├── commands/           # Package commands defines cross-module command types for game operations.
...
│   ├── headlines/          # Package headlines detects and emits derived game events — player
...
│   ├── logic/
│   │   ├── advancement/        # Package advancement provides voluntary phase advancement for attack,
...
user has joined.
│   ├── publisher/          # Package consumers contains event handlers that react to game bus events
...
│   ├── tracing/            # Package tracing provides helpers for creating OpenTelemetry spans within
...
│   ├── metrics/            # Package metrics registers OpenTelemetry instruments for infrastructure
...
│   ├── publisher/          # Package consumers contains event handlers that react to lobby bus events
