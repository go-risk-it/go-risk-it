...
- **Living Architecture** -- Every package has a [`doc.go`](docs/doc-go-spec.md) with structured sections (`# Layer`, `# Key Types`, `# Dependencies`). CI validates layer claims against the actual import graph. [41 architecture rules](internal/) enforce boundaries at test time -- import constraints, quality ratchets, and documentation coverage.
