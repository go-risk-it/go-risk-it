...
func TestTestBus_EventsOfType(t *testing.T) {
	...
	moves := events.EventsOfType[*gameevt.MoveExecuted](bus)
	require.Len(t, moves, 2)
	require.Equal(t, move1, moves[0])
	require.Equal(t, move2, moves[1])

	phases := events.EventsOfType[*gameevt.PhaseTransitioned](bus)
	require.Len(t, phases, 1)
	require.Equal(t, phase, phases[0])

	completions := events.EventsOfType[*gameevt.GameCompleted](bus)
...
func TestTestBus_HandlersCalledOnEmit(t *testing.T) {
	...
	bus.OnType(gameevt.TypeGameCreated, func(_ context.Context, event events.Event) {
