...
import (
	...
	mockWeblobby "github.com/go-risk-it/go-risk-it/mocks/internal_/lobby/web"
...
func newDeps(t *testing.T) *deps {
	...
	return &deps{
		writer:   mockWeblobby.NewWriter(t),
		stateSvc: mockState.NewService(t),
	}
}

func (d *deps) newBroadcaster() *weblobby.LobbyStateBroadcaster {
	stateCtrl := weblobby.NewStateController(d.stateSvc)

	return weblobby.NewLobbyStateBroadcaster(d.writer, stateCtrl)
}
...
func TestOnStateChanged_BroadcastsLobbyState(t *testing.T) {
	...
	pub.Register(bus)
	bus.Emit(lobbyCtx, lobbyevt.NewLobbyStateChanged(testLobbyID, testUserID, nil))
