...
import (
	...
	"github.com/go-risk-it/go-risk-it/internal/game/logic/metrics"
...
// emitGameLifecycle emits a GameCreated, then the provided events, then a GameCompleted.
func emitGameLifecycle(
	...
	bus.Emit(ctx, gameevt.NewGameCreated(testGameID, fixedTime, 4))

	for _, e := range events {
		bus.Emit(ctx, e)
	}

	bus.Emit(ctx, gameevt.NewGameCompleted(testGameID, testUser, fixedTime, testTurn))
}

func moveEvent(actionType sqlc.GamePhaseType) *gameevt.MoveExecuted {
	var attackResult *attack.MoveResult
	if actionType == sqlc.GamePhaseTypeATTACK {
...
func TestGameSummaryRecorder_AccumulatesMoveCounts(t *testing.T) {
	...
	events := []eventbus.Event{
		moveEvent(sqlc.GamePhaseTypeDEPLOY),
		moveEvent(sqlc.GamePhaseTypeATTACK),
		moveEvent(sqlc.GamePhaseTypeCONQUER),
		moveEvent(sqlc.GamePhaseTypeREINFORCE),
		moveEvent(sqlc.GamePhaseTypeCARDS),
	}
...
func TestGameSummaryRecorder_CountsAttacksOnly(t *testing.T) {
	...
	// attacks histogram should record 3 (only ATTACK moves)
	...
	require.InDelta(t, 3.0, attackHist.DataPoints[0].Sum, 0.001)

	// moves histogram should record 6 (all moves)
	moveHist, moveOk := histograms["game.summary.moves"]
...
func TestGameSummaryRecorder_RecordsOnCompletion(t *testing.T) {
	...
	events := []eventbus.Event{
		moveEvent(sqlc.GamePhaseTypeDEPLOY),
		moveEvent(sqlc.GamePhaseTypeATTACK),
		gameevt.NewPhaseTransitioned(testGameID, testUser, fixedTime,
			sqlc.GamePhaseTypeDEPLOY, sqlc.GamePhaseTypeATTACK, 1),
		gameevt.NewPhaseTransitioned(testGameID, testUser, fixedTime,
			sqlc.GamePhaseTypeATTACK, sqlc.GamePhaseTypeCONQUER, 2),
	}

	// Emit events without GameCompleted first — no histograms should be recorded.
	gCtx := gameCtx(testGameID)
	bus.Emit(gCtx, gameevt.NewGameCreated(testGameID, fixedTime, 4))
	...
	if h, ok := histogramsBefore["game.summary.moves"]; ok {
		for _, dp := range h.DataPoints {
			require.Equal(t, uint64(0), dp.Count,
				"no histogram data points should be recorded before GameCompleted")
		}
	}

	// Now emit GameCompleted — histograms should be recorded.
	bus.Emit(gCtx, gameevt.NewGameCompleted(testGameID, testUser, fixedTime, testTurn))
	...
	require.Len(t, moveHist.DataPoints, 1)
	require.Equal(t, uint64(1), moveHist.DataPoints[0].Count)
	require.InDelta(t, 2.0, moveHist.DataPoints[0].Sum, 0.001) // 2 moves

	turnHist, turnOk := histogramsAfter["game.summary.turns"]
	require.True(t, turnOk)
	require.Len(t, turnHist.DataPoints, 1)
	require.InDelta(t, 2.0, turnHist.DataPoints[0].Sum, 0.001) // 2 phase transitions
}
...
func TestGameSummaryRecorder_CleansUpOnCompletion(t *testing.T) {
	...
	moveHist, moveOk := histograms["game.summary.moves"]
	require.True(t, moveOk)
	// Should have 2 data points recorded (2 observations at different values)
	require.Equal(t, uint64(2), moveHist.DataPoints[0].Count)
	// Sum should be 2 + 1 = 3 (first game had 2 moves, second had 1)
	require.InDelta(t, 3.0, moveHist.DataPoints[0].Sum, 0.001)
}
...
func TestGameSummaryRecorder_IgnoresUnknownGame(t *testing.T) {
	...
	bus.Emit(gCtx, moveEvent(sqlc.GamePhaseTypeDEPLOY))
	bus.Emit(gCtx, gameevt.NewPhaseTransitioned(testGameID, testUser, fixedTime,
		sqlc.GamePhaseTypeDEPLOY, sqlc.GamePhaseTypeATTACK, 1))
	bus.Emit(
		gCtx,
		headlines.NewPlayerEliminated(testGameID, "victim", testUser, fixedTime, testTurn),
	)

	// Complete an unknown game — should not record anything.
	bus.Emit(gCtx, gameevt.NewGameCompleted(testGameID, testUser, fixedTime, testTurn))
...
func TestNewGameMetrics_CreatesAllHistograms(t *testing.T) {
	...
	// Record a value on each histogram to verify they exist and are functional.
